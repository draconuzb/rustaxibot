...
BLOCK_FILE = "blocked.json"   # ✅ bloklanganlar fayli
...
async def unblock_user(message: Message):
    ...
    try:
        ...
        return await message.answer("❌ Format noto‘g‘ri!\nTo‘g‘ri ishlatish: /unblock <user_id>")

    if remove_block(uid):
        try:
            await bot.send_message(uid, "✅ Sizning blokdan chiqarildingiz. Endi botdan yana foydalanishingiz mumkin.")
        except:
            pass
        await message.answer(f"✅ Foydalanuvchi {uid} blokdan chiqarildi.")
    else:
        await message.answer("⚠ Bu foydalanuvchi blokda emas!")
...
# 🔓 Blok muddati tugaganda xabar yuborish
async def check_block_expire():
    while True:
        ...
        for uid, end in list(data.items()):
            ...
            if end_date <= now:
                try:
                    await bot.send_message(int(uid), "✅ Sizning blok muddatingiz tugadi. Endi botdan yana foydalanishingiz mumkin.")
                except:
                    pass
                del data[uid]
                save_json(BLOCK_FILE, data)
